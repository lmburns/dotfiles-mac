...
notify () {
    terminal-notifier -title "Netinfo" -message "$1"
}
...
[[ "$EXTERNAL_IP4" == "None" ]] && WHOIS="" \
  || WHOIS=$(whois "$EXTERNAL_IP4" \
    | awk '/descr: / {$1=""; print $0 }' \
    | head -n 1)

# Find interfaces
INTERFACES=$(ifconfig | grep UP | grep -E -o '(^en[0-9]*|^utun[0-9]*)' | sort -n)

# Start building output
[[ "$EXTERNAL_IP4" == "None" && "$EXTERNAL_IP6" == "None" ]]  && echo "❌" || echo "🌐"
echo "---"
echo "🔄 Refresh | color=#d3869b refresh=true"
echo "---"
echo "Public: "
...
if [[ -e "$SPEEDTEST" ]]; then
     LAST="$(date -r "$SPEEDTEST" "+%Y-%m-%d %H:%M:%S")"
...
for INT in $INTERFACES; do
     ...
     ifconfig "$INT" | \
       ...
       /ether/ {
        print \"MAC: \" \$2 \" \
          | terminal=false bash='$0' param1=copy param2=\" \$2 };
      /inet / {
        print \"IPv4: \" \$2 \" \
          | terminal=false bash='$0' param1=copy param2=\" \$2 };
      /inet6/ {
        print \"IPv6: \" \$2 \" \
          | terminal=false bash='$0' param1=copy param2=\" \$2 }" \
      | sed -e 's/%utun[0-9]*//g' -e 's/%en[0-9]*//g' \
      | sort
     echo "---"
done
