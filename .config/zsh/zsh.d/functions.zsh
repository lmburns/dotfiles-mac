...
function ww() {
  (alias; declare -f) |
    /usr/local/bin/gwhich --tty-only --read-alias --read-functions --show-tilde --show-dot $@;
}
...
# Backup files
function bak()  { /usr/local/bin/gcp -vr --force --suffix=.bak $1 $1 }
function rbak() { /usr/local/bin/gcp -vr --force $1.bak $1 }

# Backup a file. 'fname' -> 'fname_2023-01-30T14:23-06:00'
function bk() {
  emulate -L zsh
  /usr/local/bin/gcp -iv -b "$1" "${1:r}_$(date --iso-8601=m)${${${1:e}:+.${1:e}}:-}"
}

# Backup a file with only a date. 'fname' -> 'fname_2023_01_30'
function bk-today() {
  emulate -L zsh
  /usr/local/bin/gcp -iv -b "$1" "${1:r}_$(date '+%Y_%m_%d')${${${1:e}:+.${1:e}}:-}"
}
